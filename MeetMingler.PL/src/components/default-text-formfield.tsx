...
export interface DefaultTextFormFieldProps<TSchema extends FieldValues> {
  name: Path<TSchema>;
  namePrettified?: string;
  placeholder?: string;
  type?: HTMLInputTypeAttribute;
  form: UseFormReturn<TSchema>;
}

export function DefaultTextFormField<TSchema extends FieldValues>(
  props: DefaultTextFormFieldProps<TSchema>
) {
  ...
  return (
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          <FormLabel>
            {props.namePrettified ?? deCamelCaseify(props.name)}
          </FormLabel>
          <FormControl>
            <Input
